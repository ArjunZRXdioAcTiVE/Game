class Game {
  ...
  detectCollisions (castle) {
    for (var index in cannonBalls) {
      for (var castlePart of castle) {
        if (cannonBalls[index].ballRemoved || castlePart.destroyed) return;

        console.log("here");  

        cannonBalls[index].dummySprite.overlap(castlePart, (collector, collected) => {
          console.log("here")
          collected.health -= 40;


          cannonBalls[index].removeBall(index)
        })
    ...
    await firebase.ref("players").once("value", data => {
      ...
      for (var player in playersInfo) {
        this.leaderBoard.html(`
          ${playersInfo[player].rank} </br>
          ${player.damageTaken} </br>
          ${player.damageDone} </br>`
        )
        .position(200, 50);
      }
    })
